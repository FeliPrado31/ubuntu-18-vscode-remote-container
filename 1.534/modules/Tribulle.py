...
class Tribulle:
    ...
    def parseTribulleCode(this, code, packet):
        if code == 28:
            ...
            if this.server.isDebug:
                print "[%s] [WARN][%s] Invalid tribulle code -> Code: %s packet: %s" %(_time.strftime("%H:%M:%S"), this.client.Username, code, repr(packet.toByteArray()))

    def platformAuthentication(this, status):
        this.client.sendPacket(Identifiers.send.New_Tribulle, ByteArray().writeBool(status).toByteArray())
    ...
    def sendFriendsList(this, readPacket):
        ...
        if this.client.marriage == "":
            ...
            p.writeInt(rs[1]).writeUTF(rs[0].lower()).writeByte(rs[2]).writeInt(rs[1]).writeByte(1).writeBool(this.server.checkConnectedAccount(rs[0])).writeInt(4).writeUTF(player.roomName if player else "").writeInt(rs[3])
        infos = {}
        ...
        for playerName in playersNames:
            ...
            if not playersNames == ['']:
                ...
                isMarriage = this.client.Username == player.marriage if player else info[2] == this.client.Username
                p.writeInt(info[0]).writeUTF(playerName.lower()).writeByte(genderID).writeInt(info[0]).writeByte(1 if isFriend else 0).writeBool(this.server.checkConnectedAccount(playerName)).writeInt(4 if isFriend and player != None else 1).writeUTF(player.roomName if isFriend and player != None else "").writeInt(info[4] if isFriend else 0)
        if readPacket == None:
            ...
            for playerName in this.client.ignoredsList:
                if not this.client.ignoredsList == ['']:
                    p.writeUTF(playerName.lower() + this.server.getPlayerTag(playerName))
        ...
        if not readPacket == None and not this.client.marriage == "":
            this.sendPacket(15 if readPacket == "0" else 29, ByteArray().writeInt(this.client.tribulleID+1).writeByte(0).toByteArray())
    ...
    def addFriend(this, readPacket):
        ...
        if not this.server.checkExistingUser(playerName):
            this.sendPacket(19, ByteArray().writeInt(tribulleID).writeByte(11).toByteArray())
        else:
            ...
            this.sendPacket(36, ByteArray().writeInt(rs[1]).writeUTF(this.client.TFMUtils.parsePlayerName(playerName)).writeByte(rs[2]).writeInt(rs[1]).writeShort(this.server.checkConnectedAccount(playerName)).writeInt(4 if isFriend else 0).writeUTF(player.roomName if isFriend and player != None else "").writeInt(rs[3] if isFriend else 0).toByteArray())
            this.sendPacket(19, ByteArray().writeInt(tribulleID).writeByte(1).toByteArray())
            if player != None:
                player.tribulle.sendPacket(35, ByteArray().writeInt(this.client.playerID).writeUTF(this.client.Username.lower()).writeByte(this.client.gender).writeInt(this.client.playerID).writeByte(1).writeByte(this.server.checkConnectedAccount(this.client.Username)).writeInt(4 if isFriend else 0).writeUTF(this.client.roomName if isFriend else "").writeInt(this.client.lastOn if isFriend else 0).toByteArray())
    ...
    def removeFriend(this, readPacket):
        ...
        if playerName in this.client.friendsList:
            ...
            if player != None:
                player.tribulle.sendPacket(35, ByteArray().writeInt(this.client.playerID).writeUTF(this.client.Username.lower()).writeByte(this.client.gender).writeInt(this.client.playerID).writeShort(1).writeInt(0).writeUTF("").writeInt(0).toByteArray())

    def sendFriendConnected(this, playerName):
        id = this.server.getPlayerID(playerName)
        if id in this.client.friendsList:
            player = this.server.players.get(playerName)
            ...
            this.sendPacket(32, ByteArray().writeUTF(player.Username.lower()).toByteArray())
    ...
    def sendFriendDisconnected(this, playerName):
        if playerName in this.client.friendsList:
            ...
            this.sendPacket(55, ByteArray().writeUTF(playerName.lower()).toByteArray())
    ...
    def sendIgnoredsList(this, readPacket):
        ...
        for playerName in this.client.ignoredsList:
            packet.writeUTF(playerName + this.server.getPlayerTag(playerName))
    ...
    def ignorePlayer(this, readPacket):
        ...
        if not this.server.checkExistingUser(playerName):
            this.sendPacket(43, packet.writeByte(11).toByteArray())
        else:
    ...
    def whisperMessage(this, readPacket):
        tribulleID, playerName, message = readPacket.readInt(), this.client.TFMUtils.parsePlayerName(readPacket.readUTF()), readPacket.readUTF().replace("\n", "").replace("&amp;#", "&#").replace("<", "&lt;")
            ...
            if playerName.startswith("*") or not this.server.players.has_key(playerName):
                ...
                if this.client.isMute:
                    if not this.client.isGuest:
                        ...
                        timeCalc = this.client.TFMUtils.getHoursDiff(muteInfo[1])
                        if timeCalc <= 0:
                            this.server.removeModMute(this.client.Username)
                        else:
                            can = False
                            this.client.sendModMute(this.client.Username, timeCalc, muteInfo[0], True)

            if can:
                player = this.server.players.get(playerName)
                if player != None:
                    if player.silenceType != 0:
                        if (this.client.privLevel >= 5 or (player.silenceType == 1 and this.checkFriend(playerName, this.client.Username))):
                            pass
                        else:
                            this.sendSilenceMessage(playerName, tribulleID)
                            return

                    if not (this.client.Username in player.ignoredsList) and not isCheck:
                        player.tribulle.sendPacket(66, ByteArray().writeUTF(this.client.Username.lower()).writeInt(this.client.langueByte+1).writeUTF(player.Username.lower()).writeUTF(message).toByteArray())
                    this.sendPacket(66, ByteArray().writeUTF(this.client.Username.lower()).writeInt(player.langueByte+1).writeUTF(player.Username.lower()).writeUTF(message).toByteArray())

                    if isCheck:
                        this.server.sendStaffMessage(7, "<V>%s<BL> está enviando mensagens no cochicho com palavras suspeitas [<R>%s<BL>]." %(this.client.Username, message))

                    if not this.server.chatMessages.has_key(this.client.Username):
                         messages = deque([], 60)
                         messages.append([_time.strftime("%Y/%m/%d %H:%M:%S"), "> [%s] %s" %(player.Username, message)])
                         this.server.chatMessages[this.client.Username] = messages
                    else:
                         this.server.chatMessages[this.client.Username].append([_time.strftime("%Y/%m/%d %H:%M:%S"), "> [%s] %s" %(player.Username, message)])
    ...
    def disableWhispers(this, readPacket):
        ...
        this.client.silenceMessage = "" if this.server.checkMessage(this.client, message) else message
    ...
    def marriageInvite(this, readPacket):
        ...
        if not this.server.checkConnectedAccount(playerName) or not this.server.checkExistingUser(playerName):
            ...
            if not this.client.Username in player.ignoredMarriageInvites:
                ...
                player.tribulle.sendPacket(38, ByteArray().writeUTF(this.client.Username).toByteArray())
                this.sendPacket(23, packet.writeByte(1).toByteArray())

    def marriageAnswer(this, readPacket):
        tribulleID, playerName, answer = readPacket.readInt(), this.client.TFMUtils.parsePlayerName(readPacket.readUTF()), readPacket.readByte()

        player = this.server.players.get(playerName)
        if player != None:
            if answer == 0:
                this.sendPacket(25, ByteArray().writeInt(tribulleID).writeByte(1).toByteArray())
                player.tribulle.sendPacket(40, ByteArray().writeUTF(this.client.Username.lower()).toByteArray())
                ...
                this.sendPacket(39, ByteArray().writeUTF(player.Username.lower()).toByteArray())
                player.tribulle.sendPacket(39, ByteArray().writeUTF(this.client.Username.lower()).toByteArray())
    ...
    def sendTribe(this, isNew):
        if this.client.tribeName == "":
            this.sendPacket(87, ByteArray().writeInt(0).writeByte(0).toByteArray())
            return

        if not this.client.tribeChat in this.client.chats:
            this.client.chats.append(this.client.tribeChat)

        this.sendPacket(27, ByteArray().writeInt(this.client.tribeChat).writeUTF("~" + this.client.tribeName.lower()).writeBytes(chr(0) * 5).toByteArray())
        this.sendPacketWholeTribe(29, ByteArray().writeInt(this.client.tribeChat).writeInt(this.client.playerID).writeUTF(this.client.Username.lower()).toByteArray())
        this.sendTribeInfo()
    ...
    def sendLoginMessageTribe(this):
        ...
        infos = {}
        this.Cursor.execute("select Username, PlayerID, Gender, LastOn, TribeRank, TribeJoined from Users where Username in (%s)" %(this.client.TFMUtils.joinWithQuotes(members)))
        for rs in this.Cursor.fetchall():
            infos[rs[0]] = [rs[1], rs[2], rs[3], rs[4], rs[5]]
    ...
    def sendTribeMemberConnected(this):
        this.sendPacketWholeTribe(88, ByteArray().writeUTF(this.client.Username.lower()).toByteArray(), True)
        this.sendPacketWholeTribe(131, ByteArray().writeInt(this.client.playerID).writeUTF(this.client.Username.lower()).writeByte(this.client.gender).writeInt(this.client.playerID).writeInt(0).writeByte(this.client.tribeRank).writeInt(1).writeUTF("").toByteArray())

    def sendTribeMemberChangeRoom(this):
        this.sendPacketWholeTribe(131, ByteArray().writeInt(this.client.playerID).writeUTF(this.client.Username.lower()).writeByte(this.client.gender).writeInt(this.client.playerID).writeInt(0).writeByte(this.client.tribeRank).writeInt(4).writeUTF(this.client.roomName).toByteArray())

    def sendTribeMemberDisconnected(this):
        this.sendPacketWholeTribe(90, ByteArray().writeUTF(this.client.Username.lower()).toByteArray())
        this.sendPacketWholeTribe(131, ByteArray().writeInt(this.client.playerID).writeUTF(this.client.Username.lower()).writeByte(this.client.gender).writeInt(this.client.playerID).writeInt(this.client.lastOn).writeByte(this.client.tribeRank).writeInt(1).writeUTF("").toByteArray())

    def sendPlayerInfo(this):
        this.sendPacket(166, ByteArray().writeInt(0).writeInt(this.client.playerID).writeInt(this.client.playerID).writeInt(this.getInGenderMarriage(this.client.Username)).writeInt(this.server.getPlayerID(this.client.marriage) if not this.client.marriage == "" else 0).writeUTF(this.client.marriage).toByteArray())
    ...
    def createTribe(this, readPacket):
        ...
        if not this.checkExistingTribe(tribeName):
            if this.client.shopCheeses >= 500:
                this.Cursor.execute("insert into Tribe  values(null, %s, '', '0', %s, '', %s, %s, 0)", [tribeName, this.TRIBE_RANKS, this.client.Username, this.server.lastChatID])
                ...
                this.setTribeHistory(this.client.tribeCode, 1, createTime, this.client.Username, tribeName)
    ...
    def tribeInvite(this, readPacket):
        ...
        if not this.server.checkConnectedAccount(playerName) or not this.server.checkExistingUser(playerName):
            ...
            if not this.client.tribeCode in player.ignoredTribeInvites:
                ...
                player.tribulle.sendPacket(86, ByteArray().writeUTF(this.client.Username.lower()).writeUTF(this.client.tribeName).toByteArray())
    ...
    def tribeInviteAnswer(this, readPacket):
        ...
        if player != None:
            ...
            if answer == 0:
                ...
                player.tribulle.sendPacket(87, ByteArray().writeUTF(this.client.Username.lower()).writeByte(0).toByteArray())
                ...
                this.setTribeHistory(this.client.tribeCode, 2, this.getTime(), player.Username, this.client.Username)
                ...
                player.tribulle.sendPacket(87, ByteArray().writeUTF(this.client.Username).writeByte(1).toByteArray())
                this.sendPacketWholeTribe(91, ByteArray().writeUTF(this.client.Username).toByteArray(), True)
                for member in members:
                    player = this.server.players.get(member)
                    if player != None:
    ...
    def changeTribeMessage(this, readPacket):
        ...
        this.setTribeHistory(this.client.tribeCode, 6, this.getTime(), message, this.client.Username)
        this.updateTribeData()
        this.sendTribeInfo()
        this.sendPacketWholeTribe(125, ByteArray().writeUTF(this.client.Username.lower()).writeUTF(message).toByteArray(), True)
    ...
    def changeTribeCode(this, readPacket):
        ...
        if mapInfo[0] == None:
            ...
            this.setTribeHistory(this.client.tribeCode, 8, this.getTime(), this.client.Username, mapCode)
            ...
            if player != None:
                ...
                if up:
                    if player.tribeRank == rankID2:
                        player.tribeRank -= 1
                        this.Cursor.execute("update users set TribeRank = %s where Username = %s", [rankID2 - 1, member])
                if down:
                    if player.tribeRank == rankID2:
                        player.tribeRank += 1
                        this.Cursor.execute("update users set TribeRank = %s where Username = %s", [rankID2 + 1, member])
    ...
    def changeTribePlayerRank(this, packet):
        ...
        if player != None:
            ...
            this.Cursor.execute("update users set TribeRank = %s where Username = %s", [rankID, playerName])
        this.setTribeHistory(this.client.tribeCode, 5, this.getTime(), playerName, str(rankID), rankName, this.client.Username)
        ...
        this.sendPacketWholeTribe(124, ByteArray().writeUTF(this.client.Username.lower()).writeUTF(playerName.lower()).writeUTF(rankName).toByteArray(), True)
    ...
    def showTribeHistory(this, readPacket):
        tribulleID, sla, sla2 = readPacket.readInt(), readPacket.readInt(), readPacket.readInt()

        history = this.getTribeHistory(this.client.tribeCode).split("|")

        packet = ByteArray()
        packet.writeInt(tribulleID)
        packet.writeShort(len(history) - 1 if history == [''] else len(history))
        for event in history:
            event = event.split("/")
            if not history == [''] and not event[1] == '':
    ...
    def leaveTribe(this, packet):
        ...
        if this.client.tribeRank == (len(this.client.tribeRanks.split(";"))-1):
            ...
            this.sendPacketWholeTribe(92, ByteArray().writeUTF(this.client.Username.lower()).toByteArray(), True)

            members = this.getTribeMembers(this.client.tribeCode)
            if this.client.Username in members:
                members.remove(this.client.Username)
                this.setTribeMembers(this.client.tribeCode, members)

                this.setTribeHistory(this.client.tribeCode, 4, this.getTime(), this.client.Username)
    ...
    def kickPlayerTribe(this, packet):
        ...
        if tribeCode != 0:
            ...
            if playerName in members:
                ...
                this.setTribeHistory(this.client.tribeCode, 3, this.getTime(), playerName, this.client.Username)
                this.sendPacketWholeTribe(93, ByteArray().writeUTF(playerName.lower()).writeUTF(this.client.Username.lower()).toByteArray(), True)
    ...
    def setTribeMaster(this, packet):
        ...
        this.Cursor.execute("update users set TribeRank = %s where Username = %s", [len(rankInfo)-2, this.client.Username])
        player = this.server.players.get(playerName)
        if player != None:
        ...
        this.sendPacket(131, ByteArray().writeInt(this.client.playerID).writeUTF(this.client.Username.lower()).writeByte(this.client.gender).writeInt(this.client.playerID).writeInt(0).writeByte(len(rankInfo)-2).writeInt(4).writeUTF(this.client.roomName).toByteArray())
    ...
    def finishTribe(this, packet):
        ...
        for member in members:
            player = this.server.players.get(member)
            if player != None:
                player.tribulle.sendPacket(93, ByteArray().writeUTF(player.Username.lower()).writeUTF(this.client.Username.lower()).toByteArray())
    ...
    def customChat(this, packet):
        ...
        if re.match("^(%s=^(%s:(%s!.*_$).)*$)(%s=^(%s:(%s!_{2,}).)*$)[A-Za-z][A-Za-z0-9_]{2,11}$", chatName):
            ...
            this.sendPacket(55, ByteArray().writeInt(tribulleID).writeByte(8).toByteArray())
        
    def chatMessage(this, packet):
        tribulleID, chatName, message = packet.readInt(), packet.readUTF(), packet.readUTF()
        chatID = this.getChatID(chatName)
        this.sendPacketWholeChat(chatID, 64, ByteArray().writeUTF(this.client.Username.lower()).writeInt(4).writeUTF(chatName).writeUTF(message).toByteArray(), True)
        this.sendPacket(49, ByteArray().writeInt(tribulleID).writeByte(1).toByteArray())
    ...
    def sendTribeChatMessage(this, readPacket):
        tribulleID, message = readPacket.readInt(), readPacket.readUTF()
        this.sendPacketWholeTribe(65, ByteArray().writeUTF(this.client.Username.lower()).writeUTF(message).toByteArray(), True)
    ...
    def getTribeHistory(this, tribeCode):
        this.Cursor.execute("select History from Tribe where Code = %s", [tribeCode])
        rs = this.Cursor.fetchone()
        if rs:
            return rs[0]
        else:
    ...
    def setTribeCache(this, tribeCode, history):
        this.Cursor.execute("update Tribe set History = %s where Code = %s", [history, tribeCode])

    def setTribeHistory(this, tribeCode, *data):
        history = this.getTribeHistory(tribeCode)
        if history == "":
            history = "/".join(map(str, data))
        else:
            history = "/".join(map(str, data)) + "|" + history
        this.setTribeCache(tribeCode, history)
