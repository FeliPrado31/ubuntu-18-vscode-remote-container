...
##                this.client.sendMessage("<ROSE>,")
        ...
        def sendFriendConnected(this, playerName):
            if playerName in this.client.friendsList:
                ...
                this.sendPacket(32, ByteArray().writeUTF(player.playerName.lower()).toByteArray())
        ...
        def sendFriendDisconnected(this, playerName):
            if playerName in this.client.friendsList:
                ...
                this.sendPacket(33, ByteArray().writeUTF(playerName.lower()).toByteArray())
                    ...
                    if can:
                        ...
                        if player != None:
                            if player.silenceType != 0:
                                if (this.client.privLevel >= 3 or (player.silenceType == 2 and this.checkFriend(playerName, this.client.playerName))):
                                    pass
                                else:
                            ...
                            if this.client.privLevel >= 1:
                                this.client.sendLuaMessageAdmin("[Susurro] [<J>%s</J>] - [<J>%s</J>] - [<J>%s</J>]  - <J>%s</J> => <J>%s</J> : <CH>%s" %(this.client.ipAddress, this.client.roomName, this.client.langue, this.client.playerName, playerName, message))
                            if "www" in message or "http" in message or ".com" in message or "https" in message:
                                this.server.sendStaffMessage(7, '<font color="#00FFA6">Posible spam de <font color="#FFFF00"> '+str(this.client.playerName)+'<font color="#00FFA6"> por SUSURRO en la sala <font color="#FFFF00">'+str(this.client.roomName.replace('es-', ''))+'<ch>: <font color="#FFFFFF">'+str(message)+'')
...
##            else:
##                 this.client.sendMessage("<ROSE>You need 3 cheeses to speak.")            
        ...
        def sendSilenceMessage(this, playerName, tribulleID):
            player = this.server.players.get(playerName)
            if player != None:
                this.sendPacket(53, ByteArray().writeInt(tribulleID).writeByte(25).writeUTF(player.silenceMessage).toByteArray())
        ...
        def marriageInvite(this, readPacket):
            ...
            if not this.server.checkConnectedAccount(playerName) or not this.server.checkExistingUser(playerName):
                ...
                this.sendPacket(23, packet.writeByte(15).toByteArray())
            else:
        ...
        def createTribe(this, readPacket):
            ...
            #if len(tribeName) > 50:
                #this.client.sendMessage("Max 50 Chararacters")
                #return
            ...
            if not this.checkExistingTribe(tribeName):
                if this.client.shopCheeses >= 500:
                    this.Cursor.execute("insert into Tribe  values(NULL, %s, '', '0', %s, '', %s, %s, 0)", [tribeName, this.TRIBE_RANKS, this.client.playerName, this.server.lastChatID])
                ...
                this.client.sendMessage("<ROSE>Tribename contains HTML.")
        ...
        def finishTribe(this, packet):
            ...
            for member in members:
                ...
                this.client.sendPacket([6, 9], ByteArray().writeUTF("Tribe distributed.").toByteArray())
        ...
        def customChat(this, packet):
            ...
            if re.match("^[ a-zA-Z0-9]*$", chatName):
                ...
                if chatID == -1:
                    ...
                    this.server.configs("ids.lastChatID", str(chatID))
                    this.Cursor.execute("insert into Chats (ID, Name) values (%s, %s)", [chatID, chatName])
        ...
        def chatMessage(this, packet):
            ...
            chatID = this.getChatID(chatName)
            this.sendPacketWholeChat(chatID, 64, ByteArray().writeUTF(this.client.playerName.lower()).writeInt(this.client.langueID+1).writeUTF(chatName).writeUTF(message).toByteArray(), True)
            this.sendPacket(49, ByteArray().writeInt(tribulleID).writeByte(1).toByteArray())
            #this.client.sendWhisperMessageAdmin(1, "[<J>%s</J>] [<J>%s</J>] [<J>CHAT</J>] - %s => %s" %(this.client.ipAddress, chatName, this.client.playerName, message))
        ...
        def sendTribeChatMessage(this, readPacket):
            ...
            #this.client.sendWhisperMessageAdmin(1, "[TRIBECHAT] [<J>%s</J>] - [<J>%s</J>] - [%s]: <CH>%s" %(this.client.ipAddress, this.client.tribeName, this.client.playerName, message))
