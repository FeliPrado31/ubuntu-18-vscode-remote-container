...
class Skills:
    ...
    def sendExp(this, level, exp, nextLevel):
        this.client.sendPacket(Identifiers.send.Shaman_Exp, ByteArray().writeShort(level - 1).writeInt(exp).writeInt(nextLevel).toByteArray())
    ...
    def sendEarnedLevel(this, playerName, level):
        this.client.room.sendAll(Identifiers.send.Shaman_Earned_Level, ByteArray().writeUTF(playerName).writeShort(level - 1).toByteArray())
    ...
    def sendSkillObject(this, objectID, posX, posY, angle):
        this.client.room.sendAll(Identifiers.send.Skill_Object, ByteArray().writeUnsignedShort(posX).writeUnsignedShort(posY).writeByte(objectID).writeUnsignedShort(angle).toByteArray())
    
    def sendShamanSkills(this, refresh):
        packet = ByteArray().writeByte(len(this.client.playerSkills))
    ...
    def sendEnableSkill(this, id, count):
        this.client.sendPacket(Identifiers.send.Enable_Skill, ByteArray().writeUnsignedByte(id).writeUnsignedByte(count).toByteArray())
    ...
    def sendGravitationalSkill(this, seconds, velX, velY):
        this.client.room.sendAll(Identifiers.send.Gravitation_Skill, ByteArray().writeShort(seconds).writeInt(velX).writeInt(velY).toByteArray())
    ...
    def sendEvolutionSkill(this, playerCode):
        this.client.room.sendAll(Identifiers.send.Evolution_Skill, ByteArray().writeInt(playerCode).writeByte(100).toByteArray())
    ...
    def getkills(this):
        if this.client.isShaman:
            ...
            if this.client.playerSkills.has_key(89) and not this.client.room.isSurvivor:
                ...
                this.sendEnableSkill(49, [56, 52, 48, 44, 40][(5 if count > 5 else count) - 1])
    ...
    def placeSkill(this, objectID, code, px, py, angle):
        if code == 36:
            ...
            for player in this.client.room.clients.values():
                if player.isDead and not player.hasEnter and not player.isAfk and not player.isShaman and not player.isNewPlayer:
                    if this.client.ambulanceCount > 0:
                        ...
                        this.client.room.respawnSpecific(player.playerName)
                        player.isDead = False
            ...
            if not this.client.room.isSurvivor:
                ...
                reactor.callLater(this.client.playerSkills[82] * 2, lambda: this.sendIceMouseSkill(player.playerCode, False))
    ...
    def checkQualifiedPlayer(this, px, py, player):
        if not player.playerName == this.client.playerName and not player.isShaman:
