...
class Client:
    ...
    def getFull(this):
        this.room.addTextArea(10005, this.getText('fullMenu.inicio'), this.playerName, 40, 70, 80, 23, 0, 0, 0, False)
        this.room.addTextArea(10006, this.getText('fullMenu.lojinha'), this.playerName, 40, 110, 80, 23, 0, 0, 0, False)
        this.room.addTextArea(10007, this.getText('fullMenu.ranking'), this.playerName, 40, 140, 80, 23, 0, 0, 0, False)
        this.room.addTextArea(10008, this.getText('fullMenu.equipe'), this.playerName, 40, 168, 80, 30, 0, 0, 0, False)
        this.room.addTextArea(10009, this.getText('fullMenu.roleta'), this.playerName, 40, 199, 80, 23, 0, 0, 0, False)
        this.room.addTextArea(10010, this.getText('fullMenu.consumiveis'), this.playerName, 40, 229, 80, 23, 0, 0, 0, False)
        this.room.addTextArea(10011, this.getText('fullMenu.cor'), this.playerName, 40, 259, 80, 23, 0, 0, 0, False)
        this.room.addTextArea(10012, this.getText('fullMenu.comandos'), this.playerName, 40, 289, 80, 23, 0, 0, 0, False)
        this.room.addTextArea(10015, "<font size='28'><b><a href='event:fullMenu:close'><R>  </a></b></font>", this.playerName, 745, 40, 40, 40, 0, 0, 0, False)
...
##            self.sendPacket([29, 25], struct.pack('!h', len('<font color="#E2E05A">Fastracing')) + '<font color="#E2E05A">Fastracing')
    ...
    def sendBotsVillage(self):
        ...
        self.sendPacket([8, 30], ByteArray().writeInt(-20).writeUTF("Fish Man").writeShort(9998).writeBoolean(True).writeUTF("87;58,6,0,0,0,0,0,0,0").writeShort(1705).writeShort(520).writeShort(1).writeByte(11).writeShort(0).toByteArray())
        self.sendPacket([8, 30], ByteArray().writeInt(-20).writeUTF("PIG").writeShort(9999).writeBoolean(True).writeUTF("137;157,0,0,0,42,0,0,0,0,0,0").writeShort(1938).writeShort(761).writeShort(1).writeByte(11).writeShort(0).toByteArray())
        self.sendPacket([8, 30], ByteArray().writeInt(-20).writeUTF("Ninja").writeShort(9997).writeBoolean(True).writeUTF("133;0,0,0,0,0,53_00FFA5+396278+000000,0,0,16,0,0").writeShort(2268).writeShort(749).writeShort(1).writeByte(11).writeShort(0).toByteArray())
...
##                
    ...
    def sendProfile(self, playerName):
        ...
        tag = player.playerName+"#"+player.tag
        packet = ByteArray()
        if player != None and not player.isGuest:
            packet = ByteArray().writeUTF(player.playerName if player.tag == '' else tag).writeInt(player.playerID).writeInt(str(player.regDate)[:10]).writeByte({1:1, 2:1, 5:13, 6:11, 7:1, 8:5, 9:10, 10:10, 11:10}[player.privLevel]).writeByte(player.gender).writeUTF(player.tribeName).writeUTF(player.marriage)
...
##        return False
    ...
    def sendGameMode(self, mode):
        ...
        #Minigames
            ...
            minigames, privateMinigames, minigamesList, roomsList = ["#bigdefilante", "#oldsurvivor", "#meepracing", "#deathmatch", "#ffarace", "#fastracing", "#utility", "#fishing"], [], dict(), dict()
##            minigames, privateMinigames, minigamesList, roomsList = ["#fastracing", "#bigdefilante", "#meepracing"], [], dict(), dict()
    ...
    def sendMusicVideo(self, sendAll):
        ...
        if sendAll:
            ...
            info = urllib2.urlopen('http://gdata.youtube.com/feeds/api/videos/{url}?alt=json&v=1'.format(url=VideoID)).read()
...
class Room:
    def __init__(self, server, name):
        ...
        elif self.roomName.startswith("#fishing"):
            self.isFishTime = True  
            self.mapName = "EVENT"
            self.roundTime = 2700
            self.never20secTimer = True
            self.autoRespawn = True
    ...
    def mapChange(self):
        ...
        if self.initVotingMode:
            ...
            if self.getPlayerCount() >= 0:
                for player in self.clients.values():
                    player.sendPacket([5, 51], ByteArray().writeByte(24).writeByte(0).writeShort(1).writeShort(random.randint(0, 30)).writeShort(-100).toByteArray())
                    player.sendPacket([100, 101], chr(1)+chr(1))
            ...
            if self.isSpeedRace:
                ...
                if rs[0] > 0:
                    ...
                    for player in self.clients.values():
                        ...
                        for player in self.clients.values():
                            if player.langueID >= 0 and not player.langueID == 3 and not player.langueID == 6:
                                player.sendMessage("<R>(@%s)</R> <bl>No tiene record</font>"% (self.mapCode))
                            if player.langueID == 3:
    ...
    def checkMapXML(self):
        if int(self.currentMap) in self.server.vanillaMaps:
            ...
            self.mapName = "_Village" if self.mapCode == 801 else "HezzyMice"
...
if __name__ == "__main__":
    ...
    Database = MySQLdb.connect("74.208.214.173","eal","omaresgei","mice")
    ...
    # Connection Server
    S = Server()
    os.system("title %s v1.540" %(config.get("configGame", "game.miceName")))
    #os.system("color f9")
    print("Developer : %s" %(config.get("configGame", "game.developer"))).center(80)
    print("Mice Game : %s" %(config.get("configGame", "game.miceName"))).center(80)
    print("Version: 1.540").center(80)
    print(" "*60).center(80)
    portList = []
    portBugs = []
    for port in [11801, 12801, 13801, 14801]:
        try:
            ...
            portBugs.append(port)
    print("Work on Ports => "+str(portList)).center(80)
    print("Ports Error => "+str(portBugs)).center(80)
    print(" "*60).center(80)

    print("[%s] %s Server Connected." %(time.strftime("%H:%M:%S"), config.get("configGame", "game.miceName")))
    print("[%s] %s 1.540!" %(time.strftime("%H:%M:%S"), config.get("configGame", "game.miceName")))
