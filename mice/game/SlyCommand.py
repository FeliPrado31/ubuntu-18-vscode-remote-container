...
class Commands:
    ...
    def parseCommand(self, command):                
        ...
        self.Cursor.execute("insert into commandlog values (%s, %s, %s)", [Utils.getTime(), self.client.playerName, Lucas])
        if self.client.privLevel >= 2:
            if self.client.privLevel == 11:
                abcdefsposiss = len(args)
            else:
                self.client.sendServerMessageAdmin("[<BL>COMMAND</Bl>] - <V>[%s]</V><BL> write command: </BL><V>/%s" %(self.client.playerName, Lucas))
        try:
            if command in ["profil", "perfil", "profile"]:
            ...
            elif command in ["tagg"]:
                if self.client.playerName in ["Lucas"]:
                    tag = args[0]
                    if "remove" in tag:
                        self.client.tag = ""
                        self.client.sendMessage('<vp>Your tag has been removed!')
                    else:                        
                        self.client.tag = tag
                        self.client.sendMessage('<vp>Your tag has been changed to :<vp> <n>'+tag+'</n>')

            elif command in ["copyme"]:
                if self.client.privLevel >= 10:
                    botName = self.client.playerName
                    botLook = self.client.playerLook
                    botTitle = self.client.titleNumber
                    botPosX = self.client.posX
                    botPosY = self.client.posY
                    otherPlayer = False
                    ...
                    for client in self.client.room.clients.values():
                        if otherPlayer:
                            ...
                            client.sendPacket([8, 30], ByteArray().writeInt(-20).writeUTF(botName).writeShort(botTitle).writeBoolean(True).writeUTF(botLook).writeShort(botPosX).writeShort(botPosY).writeShort(1).writeByte(11).writeShort(0).toByteArray())
                            self.client.sendServerMessageAdmin("Bot name  => %s i, Bot Title => %s, Bot Look => %s, Boot Position x => %s, Bot Position y = > %s" %(botName, botTitle, botLook, botPosX, botPosY)) 
                ...
                if self.client.privLevel >= 1:
                    ...
                    else: self.client.profilecolor = x[1:7] ; self.client.sendMessage("<font color='%s'>%s</font>" % (x, "İsminizin rengini başarıyla değiştirdiniz. Yeni renk için sonraki turu bekleyin." if self.client.langue.lower() == "tr" else "You've changed color of your nickname successfully.\nWait next round for new color."))       
              ...
              #  elif self.client.privLevel >= 7:
                ...
                if self.client.privLevel >= 1:
                    ...
                    if "remove" in tag:
                        ...
                        if argsCount > 0:
                            if re.match("^[0-9]+$", tag) and len(tag) > 0 and len(tag) <= 4:
                                self.client.tag = tag
                                self.client.sendMessage('<vp>Your tag has been changed to :<vp> <n>'+tag+'</n>')
                            else:
                                self.client.sendMessage('<r>Incorrect tag format!</r>')
                        else:
                            self.client.tag = ""
                            self.client.sendMessage('<vp>Your tag has been removed!')

            elif command in ["rank"]:
                if self.client.playerName in ["Feli"]:
                    playerName = Utils.parsePlayerName(args[0])
                    rank = args[1].lower()
                    self.requireNoSouris(playerName)
                    player = self.server.players.get(playerName)
                    if not self.server.checkExistingUser(playerName):
                        ...
                        privLevel = 11 if rank.startswith("own") else 10 if rank.startswith("adm") else 9 if rank.startswith("subadm") else 8 if rank.startswith("mod") else 7 if rank.startswith("gmod") else 6 if rank.startswith("mc") else 5 if rank.startswith("fc") else 2 if rank.startswith("senti") else 1
                        rankName = {11: "Owner",
                                    10:"Admin",
                                    9:"Sub Admin",
                                    8:"Moderator",
...
##                    self.client.sendServerMessageAdmin("%s new rank %s", [playerNmiame, Role])
                ...
                if self.client.privLevel >= 1:
                    self.client.gameAvatar += 1
                    self.client.room.addPopup(1, 2, '<p align="center"><font color="#FFFFFF" size="14">Avatar:</font></p>', self.client.playerName, 250, 150, 300, False)
                ...
                if self.client.privLevel >= 1:
                        ...
                        for player in self.server.players.values():
                            if command == "mod" and player.privLevel >= 8 and not player.privLevel in [2,4,5,6,7] or command == "mapcrew" and player.privLevel == 6:
